...
<body>
    ...
    <h3>Thống kê Sach theo tháng</h3>
    <form action="{{route("thongke.sach")}}" method="get">
        <select name="thangthue">
            <?php for($thang = 1; $thang< 12;$thang++){?>
                <option value="<?= $thang ?>" {{$thang == $thangthue ? "selected" : ""}}>Tháng <?= $thang?></option>
            <?php } ?>
        </select>
    ...
    <table class="table table-bordered">
        ...
        <?php for ($i = 0; $i < count($thongkesach); $i++) { ?>
            <tr>
                <td><?= $i + 1 ?></td>
                <td><?= $thongkesach[$i]->tensach ?></td>
                <td><?= $thongkesach[$i]->soluongthue ?></td>
            </tr>
        <?php } ?>
    </table>
